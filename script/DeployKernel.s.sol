...
contract DeployKernel is Script {
    ...
    address constant ENTRYPOINT_0_6 =
    ...
    function run() public {
        ...
        if (EXPECTED_KERNEL_ADDRESS.code.length == 0) {
            ...
            console.log("Kernel address1: %s", address(kernel));
        } else {
            kernel = Kernel(EXPECTED_KERNEL_ADDRESS);
            console.log("Kernel address2: %s", address(kernel));
        }

        KernelFactory factory;
        if (EXPECTED_KERNEL_FACTORY_ADDRESS.code.length == 0) {
            factory = new KernelFactory{salt: 0}(
                DEPLOYER,
